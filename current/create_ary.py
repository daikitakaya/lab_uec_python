...
pair_ary = np.ones([NEURON_NUM,NEURON_NUM])
...
for t in times:
  I = np.zeros(NEURON_NUM) + 12.0 + np.random.uniform(-5.0,5.0,NEURON_NUM) #t秒における60個のニューロンへの入力(配列)
  sum_input_output01.write(str(t) + "\t" + str(S[0]) + "\n")
  v_output.write(str(t) + "\t" + str(V[0]) + "\n")
  for num_i in range(NEURON_NUM):#ニューロンごとの処理
    for connect_num in [i for i,j in enumerate(pair_ary[num_i,:]) if j == 1.0]: #接続している他ニューロンでループを回す(), connect_numはnum_iと接続しているニューロン
      if V[connect_num] >= V_th: #接続しているニューロン(j番目)が発火して入れば
        s_ij[connect_num][num_i] += 1.0 #i番目のニューロンへの入力を1mV増加
        S[num_i] += W0 * s_ij[connect_num][num_i] #接続しているニューロンからの入力の総和
    S[num_i] = S[num_i] * math.exp(-(t - firing_times[num_i]) / 30.0)# 他ニューロンからの入力の総和の減衰
  for num_i in [i for i,j in enumerate(V) if j >= V_th]:#15mV越える時
    ...
    if num_i == 0:
      ...
      v_output.write(str(t) + "\t" + str(V[0]) + "\n")
    V[num_i] = V_re
  for num_i in [i for i,j in enumerate(V) if j < V_th]:#ルンゲで値更新
    V[num_i] = runge(V[num_i], I[num_i], S[num_i])
